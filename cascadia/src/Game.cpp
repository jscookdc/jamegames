...
void Game::progressGameState(GameState game_state) {
  LOG_DEBUG << "Progressing game state from " << m_current_game_state << " to "
            << game_state;
...
void Game::enterTurnLoop() { m_current_turn++; }

void Game::initialize() {
  LOG_DEBUG << "Initializing";
  for (int i = 0; i < int(m_players.size()); i++) {
    Biome starting_biome = BiomeUtilities::getRandomBiome();
    WildlifeType starting_wildlife_type =
        WildlifeTypeUtilities::getRandomWildlifeType();
    Tile origin_tile =
        Tile(starting_biome, starting_biome, starting_wildlife_type);
  }
